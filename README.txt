...
- X_Regression_Coda_Regression.Rmd - The ML code in this manuscript was used for classification, since ages were in four discrete categories, diet was two discrete categories, so body weight was turned into a classifier by taking the top and bottom 40% within each dietary group and turned into "light" and "heavy" categories. However, body weight - or other dependent variables - could be treated as continuous variables, in which case regression models should be used instead of classification models. We include here the code for regression analysis, here implemented for body weight.
